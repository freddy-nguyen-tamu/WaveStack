...
BEGIN
  NEW.normalized_search := lower(translate(unaccent(concat_ws(' ',
    NEW.file_name, NEW.title, NEW.artist_name, NEW.album_title, NEW.lyrics)), 'Đđ', 'Dd'));
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
UPDATE drive_tracks SET normalized_search = lower(translate(unaccent(concat_ws(' ',
  file_name, title, artist_name, album_title, lyrics)), 'Đđ', 'Dd'))
WHERE normalized_search IS DISTINCT FROM lower(translate(unaccent(concat_ws(' ',
  file_name, title, artist_name, album_title, lyrics)), 'Đđ', 'Dd'));
